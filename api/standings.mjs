...
function fifaPts(row) {
  return row.wins * 3 + row.draws;
}
function maxFifaPts(row) {
  return fifaPts(row) + (3 - row.matches) * 3;
}
function h2hResult(teamId, opponentId, fixtures) {
  const f = fixtures.find(
    (x) => x.status.type === "finished" && x.round.toLowerCase().includes("group") && (x.homeTeam.id === teamId && x.awayTeam.id === opponentId || x.homeTeam.id === opponentId && x.awayTeam.id === teamId)
  );
...
function isPositionSecured(row, groupRows, fixtures, targetPosition) {
...
function applyGroupBonuses(teamPoints, standings, fixtures) {
  ...
  const r32TeamIds = /* @__PURE__ */ new Set();
...
function playerBreakdown(stats) {
  const items = [];
...
function keeperBreakdown(stats) {
  const items = [];
...
async function readGoalState() {
  ...
  return {
    ...
    goalTeam: data?.goal_team ?? void 0,
    goalAt: data?.goal_at ?? void 0,
    prevScores: data?.prev_scores ?? {}
  };
}
async function writeGoalState(goalSeq, goalTeam, goalAt, prevScores) {
  const sb = makeSupabase();
  if (!sb) return;
...
async function computeStandings(entries2) {
  ...
  for (const e of played) {
    ...
    if (now.h > before.h) {
      ...
      goalAt = (/* @__PURE__ */ new Date()).toISOString();
  ...
  const pickToTeam = /* @__PURE__ */ new Map();
  const pickGroupBonus = /* @__PURE__ */ new Map();
...
async function handler(_req, res) {
  ...
  try {
    ...
    const live = payload.liveMatchCount > 0 || payload.allFixtures.some((f) => {
      const msUntil = f.startTimestamp * 1e3 - Date.now();
      return msUntil >= 0 && msUntil < 60 * 1e3;
    });
